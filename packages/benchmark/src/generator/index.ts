...
import { generateValidationDataset, writeValidationData } from "./generateValidationData.js";
import { purchaseBenchmarkTickets } from "./purchaseTickets.js";

// ── Environment validation ────────────────────────────────────────────────────

const JWT_SECRET = process.env.JWT_SECRET;
const SALT_ROUNDS = parseInt(process.env.SALT_ROUNDS ?? process.env.saltrounds ?? "10", 10);
...
function header(_step: number, label: string): void {
    const _pad = "─".repeat(Math.max(0, 52 - label.length));
}

function ok(_msg: string): void {}

function warn(_msg: string): void {}
...
async function main(): Promise<void> {
    ...
    ok(
        `Organiser ready: ${c.cyan(organiser.email)} ${c.dim(`(id: ${organiser.id}) ${formatDuration(Date.now() - t1)}`)}`,
    );
    ...
    ok(
        `Verifiers ready: ${c.cyan(String(verifiers.length))}  ${c.dim(formatDuration(Date.now() - tv))}`,
    );
    ...
    process.stdout.write("\n");
    ok(
        `Wallets ensured: ${c.cyan(String(users.length))}  ${c.dim(formatDuration(Date.now() - t4))}`,
    );
    ...
    ok(
        `Validation payloads: ${c.cyan(String(payloads.length))}  ${c.dim(formatDuration(Date.now() - t7))}`,
    );
    ...
    process.stdout.write("\n");
    ok(
        `JWT tokens generated: ${c.cyan(String(records.length))}  ${c.dim(formatDuration(Date.now() - t8))}`,
    );
    ...
    // ── Summary ──────────────────────────────────────────────────────────────
    const _totalElapsed = Date.now() - globalStart;
}
